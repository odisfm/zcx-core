...
Without defining any [gestures](../command-reference.md#gestures), the param control will toggle its mapped parameter from "on" to "off" on a press.
...
Follows the same format as [encoder mapping definitions](../encoder-reference.md#configuration).
All [encoder mapping targets](../encoder-reference.md#mapping-targets) are available to param controls, as well as the following additional options:
