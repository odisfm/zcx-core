...
class Push1Display(ZCXPlugin):
    ...
    def write_message_to_line(self, msg: str, line_number=None, timeout: float = 0.0):
        ...
        if len(ascii_message) < 68:
            ascii_message = ascii_message + tuple(
                32 for _ in range(68 - len(ascii_message))
            )
    ...
    def update_display_segment(self, line_num, seg_num, msg, smart_truncate=True):
        ...
        new_message_portion = self.splice_tuple(
            message_portion, start_i, end_i, new_portion_bytes
        )
    ...
    def multi_segment_message(self, line_num, start_segment, end_segment, msg):
        ...
        new_message_portion = self.splice_tuple(
            message_portion, start_i, end_i, msg_bytes
        )
    ...
    def update_selected_line(self):
        ...
        self.multi_segment_message(
            self._selected_line, 4, 7, f"s: {scene_num + 1}{scene_name_format}"
        )
