...
def get_named_color(name, calling_control=None):
    ...
    if "shade" in name.lower():
        if "${" in name:
            ...
            parse = resolver.compile(
                name, calling_control._vars, calling_control._context
            )
            if parse[1] != 0:
    ...
    try:
        ...
        try:
            ...
            if 0 <= color <= 127:
                ...
                raise ConfigurationError(
                    f"Int color value must be in range 0-127. You entered {color}."
                )
        ...
        if type(color) is str:
            if "${" in color:
                ...
                resolver = root_cs.component_map["ActionResolver"]
                parse = resolver.compile(
                    color, calling_control._vars, calling_control._context
                )
                if parse[1] != 0:
            ...
            if isinstance(special_color_def, str) and "${" in special_color_def:
                ...
                resolver = root_cs.component_map["ActionResolver"]
                parse = resolver.compile(
                    special_color_def, calling_control._vars, calling_control._context
                )
                if parse[1] != 0:
                    raise ConfigurationError(f"Unparseable color definition: {color}")
            ...
            if special_color_type == "blink":
                ...
                if len(split) == 1:
                    ...
                    raise ConfigurationError(
                        f"Invalid color definition: {special_color_def}"
                    )  # todo
                ...
                if palette_list is None:
                    raise ConfigurationError(
                        f"No palette definition in `zcx/hardware/colors.py` for {palette_name}"
                ...
                color_index = hardware_colors.live_index_for_midi_index(
                    special_color_def
                )
        ...
        if calling_control is None:
            ...
            ROOT_LOGGER.error(
                f"Invalid color definition for control {calling_control.name}:"
            )
