        ...
        try:
            try:
                ...
                if (this_live_version[0] > REQUIRED_LIVE_VERSION[0]) or (
                    this_live_version[0] == REQUIRED_LIVE_VERSION[0]
                    and this_live_version[1] >= REQUIRED_LIVE_VERSION[1]
                ):
                    pass
                else:
                    this_version_string = (
                        f"{this_live_version[0]}.{this_live_version[1]}"
                    )
                    required_version_string = (
                        f"{REQUIRED_LIVE_VERSION[0]}.{REQUIRED_LIVE_VERSION[1]}"
                    )
                    raise ZcxStartupError(
                        f"zcx requires Live version {required_version_string} or above.",
                ...
                self.__refresh_on_all_sysex = user_prefs.get(
                ...
                if initial_hw_mode == "zcx" and USER_MODE is not None:
                    if INIT_DELAY > 0:
                        ...
                        sysex_task = DelayedSysexTask(
                            self, duration=delay, sysex_tuple=USER_MODE
                        )
            ...
            try:
                ...
                if e.boilerplate:
                    popup_string += (
                        f"{self.name} encountered a fatal error while starting."
                    )
