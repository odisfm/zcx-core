...
class ActionResolver(ZCXComponent):
    ...
    def __init__(
        ...
        **k,
        ...
        self.__page_manager: PageManager = self.canonical_parent.component_map[
            "PageManager"
        ]
        self.__mode_manager: ModeManager = self.canonical_parent.component_map[
            "ModeManager"
        ]
    ...
    def __build_symtable(
        self, context: Dict[str, Any], prior_resolved: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if calling_control is None:
                ...
                parse_color = partial(
                    parse_color_definition, calling_control=calling_control
                )
        ...
        exec_context = make_symbol_table(
            **dot_context, **prior_resolved, **self.__standard_context
        )
        return exec_context
    ...
    def compile(
        ...
        if not isinstance(action_string, str) or (
            "@{" not in action_string and "${" not in action_string
        ):
    ...
    def _compile_and_check(
        self, command_str: str, vars_dict: dict = None, context: dict = None
    ):
        """Helper to compile commands and handle errors consistently"""
        parsed, status = self.compile(command_str, vars_dict, context)
        if status != 0:
            error_msg = (
                f"Couldn't parse command: {command_str}\n" f"{context}," f"{vars_dict}"
            )
            self.log(error_msg)
            if ABORT_ON_FAILURE:
            ...
            for command in commands:
                if isinstance(command, str):
                    ...
                    match command_type:
                        case "cxp":
                            if (
                                parsed := self._compile_and_check(
                                    command_def, vars_dict, context
                                )
                            ) is not None:
                                self.__cxp.trigger_action_list(parsed)
                        case "log":
                        ...
                        case "hardware_mode":
                            if command_def == "user":
                                ...
                                raise RuntimeError(
                                    f"invalid hardware mode: {command_def}"
                                )
                        ...
                        case "ring":
                            ...
                            if isinstance(command_def, str):
                                match (command_def.lower()):
                                    ...
                                    case _:
                                        raise ValueError(
                                            f"Invalid ring command: {command_def}"
                                        )
                                ...
                                if track_def is not None:
                                    track_def_parsed = self._compile_and_check(
                                        track_def, vars_dict, context
                                    )
                                else:
                                    x_def = command_def.get("x", 0)
                                    x_parsed = self._compile_and_check(
                                        x_def, vars_dict, context
                                    )

                                if scene_def is not None:
                                    scene_def_parsed = self._compile_and_check(
                                        scene_def, vars_dict, context
                                    )
                                else:
                                    y_def = command_def.get("y", 0)
                                    y_parsed = self._compile_and_check(
                                        y_def, vars_dict, context
                                    )
                        ...
                        case "color":
                            if isinstance(command_def, str):
                                command_def = self._compile_and_check(
                                    command_def, vars_dict, context
                                )
                        ...
                        case "python":
                            if (
                                ...
                                try:
                                    resolved_vars, status = self._resolve_vars(
                                        vars_dict or {}, context, "live"
                                    )
                                    if status != 0:
                                        return False

                                    exec_context = self.__build_symtable(
                                        context, resolved_vars
                                    )

                                    self.__interpreter.symtable = exec_context

                                    result = self.__interpreter.eval(
                                        parsed, raise_errors=True
                                    )
                                    ...
                                    error_msg = (
                                        f"Failed to execute code: {parsed}\nError: {e}"
                                    )
